...
class FootbotController extends Controller
{
    ...
    {
        $this->telegram = new Api(env('TELEGRAM_FOOTBOT_TOKEN'));
    ...
    public function setWebHook()
    {
        $response = $this->telegram->setWebhook([
            'url' => Url::secure(env('TELEGRAM_FOOTBOT_TOKEN') . '/webhook'),
    ...
    )
    {
        ...
        foreach ($players as $k => $player) {
            ...
            if (!empty($myPhotos) && $myPhotoFileId = $myPhotos[0][0]['file_id']) {
                ...
                $myPhoto =
                    'https://api.telegram.org/file/' . env('TELEGRAM_FOOTBOT_TOKEN') . '/' .
